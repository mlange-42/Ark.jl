...
function setup_world_get_1_access(n_entities::Int)
    world = World(Position, Velocity)
...
function benchmark_world_get_1_access(args, n)
    entities, world = args
    sum = 0.0
    for e in entities
        pos, = access_components(world, e, Val.((Position,)))
...
for n in (100, 10_000)
    SUITE["benchmark_world_get_1_access n=$(n)"] =
        @be setup_world_get_1_access($n) benchmark_world_get_1_access(_, $n) evals = 100 seconds = SECONDS
end
